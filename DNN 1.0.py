...
# 构建模型架构图
model = create_model(input_shape=100, num_classes=10)  # 示例输入形状和类别数
...
# 定义不同的模型
model_ethnicity = create_model(input_shape=100, num_classes=10)
model_gender = create_model(input_shape=100, num_classes=10)
model_age = create_model(input_shape=100, num_classes=10)

# 示例训练和测试数据
X_train_ethnicity = np.random.rand(800, 100)
y_train_ethnicity = np.random.randint(0, 10, size=(800, 10))
X_test_ethnicity = np.random.rand(200, 100)
y_test_ethnicity = np.random.randint(0, 10, size=(200, 10))

X_train_gender = np.random.rand(800, 100)
y_train_gender = np.random.randint(0, 10, size=(800, 10))
X_test_gender = np.random.rand(200, 100)
y_test_gender = np.random.randint(0, 10, size=(200, 10))

X_train_age = np.random.rand(800, 100)
y_train_age = np.random.randint(0, 10, size=(800, 10))
X_test_age = np.random.rand(200, 100)
